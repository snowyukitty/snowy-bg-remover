...
from snowy_bg_remover.decontaminate import (
    bleed_edge_rgb_from_opaque,
    estimate_foreground_rgb,
)
...
def test_estimate_foreground_rgb_keeps_opaque_pixels() -> None:
    image = Image.new("RGBA", (5, 5), (240, 240, 240, 255))
    image.putpixel((2, 2), (10, 120, 200, 255))
    image.putpixel((2, 1), (130, 180, 220, 128))
    alpha = np.zeros((5, 5), dtype=np.float32)
    alpha[2, 2] = 1.0
    alpha[1, 2] = 0.5

    result = estimate_foreground_rgb(image, alpha, fallback_radius=2)

    assert result.getpixel((2, 2)) == (10, 120, 200, 255)
    assert result.getpixel((2, 1))[3] == 128
    assert result.getpixel((0, 0))[:3] == (0, 0, 0)
